...
This is a **course challenge scaffold**, not a running application. The deliverable is a Claude Code skill named `refactor-arch` (at `.claude/skills/refactor-arch/`) that analyzes, audits, and refactors *any* codebase into MVC — technology-agnostic. The three subdirectories are **legacy target projects** the skill must work against; they are intentionally full of anti-patterns and are inputs, not code to be improved by hand. Read `README.md` (Portuguese) for the full spec, severity scale (CRITICAL/HIGH/MEDIUM/LOW), and acceptance criteria.
